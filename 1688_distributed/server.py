...
""" 你的 APPID AK SK """
APP_ID = '10322600'
API_KEY = 'Yqd8kcSYZso1pzZoVjyb4vp5'
SECRET_KEY = 'CCVLKOcF4hC6y8zpOivMcFcvQKMTTVD8'
...
if __name__ == '__main__':
    ...
    manager = QueueManger(address=("10.36.132.202", 8000), authkey="123456".encode("utf-8"))
    manager.start()
    ...
    while True:
        if not result.empty():
            ...
            time.sleep(100)#超过100秒为空表示结束
            if result.empty():
                break
    allList.sort()
    allList.reverse()
    rankList = allList[:10]#对评论数量进行排名，取前100名
    ...
    try:
        for text in allList:
            mydb.insert({"物品信息":text[2],"评价数量":text[0],"评价情感分析":round(text[3]*5,1),"评价详情":text[1]})
    except:
        pass
    manager.shutdown()
