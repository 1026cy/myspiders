...
class StockSpider(RedisSpider):
    ...
    def parse(self,response):
        ...
        for i in range(1, 4):
            ...
            # elem = driver.find_element(By.XPATH, rule)
            t=threading.Thread(target=self.fun, args=(rule,driver))
            t.start()
            t.join()
        for i in range(len(self.newsTitles)):
            print("开始存入：")
            stockitem = example.items.StockItem()
            stockitem["newsTitle"] = self.newsTitle[i]
            stockitem["newsUrl"] = self.newsUrl[i]
            print("结束")
            yield stockitem
